    ...
    math.getChance = function (data) {
        ...
        try {
            ...
            if (nodes.length > MAX_N && freeCards > 1) {
                return {percent: getChance3(data) * 100, experimental: true};
            }
        ...
        // Get chance
        var chance = {percent: prob * 100, experimental: false};

        // Return it
        var percent = chance.percent;
        var experimental = chance.experimental;
        if (percent >= 99.9 && percent < 100) {
            return {percent: 99.9, experimental: experimental}; // Avoid rounding to 100
        } else {
            return {percent: percent, experimental: experimental};
        }
    };
